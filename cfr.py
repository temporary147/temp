...
HTTP_TIMEOUT    = int(_g("HTTP_TIMEOUT_MS",        required=False, default="7000"))
DEBUG_API       = _g("DEBUG_API", required=False, default="true").lower() in ("true", "1")
JOB_TIMEOUT     = int(_g("GLOBAL_JOB_TIMEOUT_MS",  required=False, default="75000"))
# Per-adapter hard cap: one slow exchange can't block the whole job
ADAPTER_TIMEOUT = int(_g("ADAPTER_TIMEOUT_MS",     required=False, default="30000"))
    ...
    for attempt in range(1, 4):
        try:
            ...
            if attempt >= 3:
                ...
                return None
            await asyncio.sleep(0.05 * attempt + random.uniform(0, 0.05))


# FIX: Old _tryv was sequential — if variant-1 timed out (7s), THEN variant-2 was tried.
# With 2-3 variants per coin × 25 coins, this caused cascading delays stacking up.
# New _tryv fires ALL variants in parallel and returns the first valid result immediately,
# then cancels remaining tasks — much faster when some variants are slow or unresponsive.
async def _tryv(variants: List[Dict[str, Any]]) -> Optional[float]:
    if not variants:
        return None

    async def _one(v: Dict[str, Any]) -> Optional[float]:
    ...
    tasks = [asyncio.create_task(_one(v)) for v in variants]
...
# FIX: Was sequential for-loop with asyncio.sleep(0.08) per coin → now fully parallel
@_reg("huobi")
async def _a_huobi() -> AR:
    async def _f(coin: str):
        code = f"{coin}-USDT"
        r = await _fetch(_E5.format(symbol=quote(code)), f"huobi:{code}")
        if r and r.get("status") == "ok" and isinstance(r.get("data"), dict):
            return {coin: _n(r["data"].get("funding_rate") or r["data"].get("estimated_rate"))}
        return {coin: None}
    results = await asyncio.gather(*(_f(c) for c in C))
    return {k: v for r in results for k, v in r.items()}
...
async def run_all_adapters(cache_key: str = CACHE_KEY) -> Dict[str, Any]:
    ...
    adapter_cap = ADAPTER_TIMEOUT / 1000

    # FIX: Each adapter now wrapped in asyncio.wait_for(timeout=adapter_cap).
    # Previously one hanging exchange would consume the full JOB_TIMEOUT.
    # Now each exchange gets at most ADAPTER_TIMEOUT_MS before being cut off.
    async def _run(key: str) -> Dict[str, Any]:
        try:
            result = await asyncio.wait_for(_AD[key](), timeout=adapter_cap)
            return {"key": key, "result": result}
        except asyncio.TimeoutError:
            _logger.warning("Adapter [%s] timed out after %.1fs — skipping", key, adapter_cap)
            return {"key": key, "result": {c: None for c in C}}
        except Exception as exc:
            _logger.warning("Adapter [%s] raised %s — skipping", key, exc)
            return {"key": key, "result": {c: None for c in C}}
    ...
    _logger.info("All adapters finished in %.0fms", duration_ms)
