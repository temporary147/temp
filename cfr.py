...
# FIX: Reduced default timeout from 15000ms → 8000ms to fail faster on slow GH runners
...
# FIX: Reduced global job timeout from 200000ms → 90000ms; all adapters run in ~30s now
JOB_TIMEOUT = int(_g("GLOBAL_JOB_TIMEOUT_MS", required=False, default="90000"))
        ...
        if not _session or _session.closed:
            ...
            _session = aiohttp.ClientSession(
                connector=aiohttp.TCPConnector(
                    limit=200,  # FIX: increased from 100 → 200 for more parallel connections
                    ttl_dns_cache=300,
    ...
    for attempt in range(1, 4):
        try:
            ...
            if attempt >= 3:
                ...
                return None
            # FIX: Reduced retry sleep from 0.1*attempt → 0.05*attempt to speed up retries
...
# FIX: Huobi — removed sequential loop + asyncio.sleep(0.08) which was adding ~2s+ delay.
# Now runs all 25 coins in parallel using asyncio.gather(), same as other adapters.
@_reg("huobi")
